...
There's one more form of tree visualization we'll use for drawing derivation trees; this will be covered in \Cref{ch:deriv}.
...
\begin{example}
  ...
  \begin{center}
    ...
    \end{forest}
  \end{center}
  We might recognize that the leaves of this tree define a prime factorization of the root, e.g., $80 = 5 * 2 * 3 * 2$.
\end{example}
%
...
What we've done so far is more general than necessary\footnote{Sorry, I can't help it.}, and is ultimately in service of defining the \textit{derivation induction principle}.
First, observe that the set of derivations in a given derivation system $\mathcal D$, as defined in \Cref{ch:deriv}, is an inductively-define subset of trees.
...
\begin{prooftree}
  ...
  \TrinaryInfC{$J$}
\end{prooftree}
is (an instance of) an inference rule of $\mathcal D$.
...
And this is ultimately what we care about when we reason about things like type safety and meta-theory.
All said, we can state the derivation induction principle as follows:

\begin{quote}
  \textit{
    Given a property $P$ of judgments of $\mathcal D$, if
    \begin{itemize}
...
Consider the following basic derivation system over judgments of the form \enquote{$n$ is even} where $n \in \N$.
\begin{center}
  \AxiomC{}
  \RightLabel{(zero)}
  \UnaryInfC{$0$ is even}
  ...
  \AxiomC{$n$ is even}
  \RightLabel{(addTwo)}
  \UnaryInfC{$n + 2$ is even}
  \DisplayProof
\end{center}
We'd like to prove that this derivation system is sound, i.e., that if \enquote{$n$ is even} is derivable, then $n$ is, in fact, even (in particular, \enquote{$3$ is even} is not derivable).
...
But, in order to avoid ballooning this appendix into a full-blown chapter, I'll relegate the presentation of more interesting examples to \Cref{todo} and our discussion of type safety.
