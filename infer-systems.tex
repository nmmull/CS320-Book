...
Of the sequences of characters we can type, only a handful of them are valid \textit{sequences of tokens} in our programming language.
...
\begin{itemize}
\item formally define \textbf{inference rules}, i.e, the rules which govern when we can derive new judgments from judgements we've already derived;
\item use \textbf{derivations} to demonstrate that a judgment is derivable in an inference system;
\item present an extended example which consists of a collection of inference systems that define the syntax, typing, and (dynamic) semantics of a simple programming language for arithmetic.
\end{itemize}

\section{Inference Rules}

An \textbf{inference system} is determined by a collection of \textbf{inference rules}.
These inference rules describe what new information we can \textit{infer} given previously inferred information.
...
We think of judgments as the \enquote{things we can say} when we inferring something.
...
In what follows we \hl{highlight} side conditions to make clear that they are not judgments.
}
...
\begin{definition}
  An \textbf{inference rule} named \textsc{ruleName} is a nonempty sequence of parameterized judgments $J_1, \dots J_k, J_{k + 1}$ along with a collection of statements $S_1, \dots, S_l$ depending on the parameters used in those judgments.
...
\end{definition}
We call the judgments above the line \textbf{antecedences} and the judgment below the line the \textbf{consequent}.
...
\begin{itemize}
\item \textsc{zeroS} says $\codel 0$ is a well-formed expression, no matter what.
\item \textsc{oneS} says $\codel 1$ is a well-formed expression, no matter what.
\item \textsc{addS} says if $e_1$ and $e_2$ are well-formed expressions, then so is $\codel( \ \codel+ \ e_1 \ e_2 \ \codel)$.
\item \textsc{mulS} says if $e_1$ and $e_2$ are well-formed expressions, then so is $\codel( \ \codel* \ e_1 \ e_2 \ \codel)$.
\item \textsc{eqS} says if $e_1$ and $e_2$ are well-formed expressions, then so is $\codel( \ \codel= \ e_1 \ e_2 \ \codel)$.
\item \textsc{condS} says if $e_1$, $e_2$, and $e_3$ are well-formed expressions, then so is $\codel( \ \codel ? \ e_1 \ e_2 \ e_3 \ \codel)$.
\end{itemize}
